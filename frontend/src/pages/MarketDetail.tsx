...
import { Bookmark, Share2, TrendingUp, Clock, AlertCircle, Lock } from "lucide-react";
...
const MarketDetail = () => {
    ...
    useEffect(() => {
        if (isSuccess && hash) {
            toastTxSuccess(hash, "Bet placed successfully!");
            setBetAmount("");
            refetch();
            reset();
        }
    ...
    const handlePlaceBet = async () => {
        ...
        try {
            await placeBet(marketId, selectedOption, betAmount);
        } catch (err: any) {
    ...
    const handleRefund = async () => {
        ...
        try {
            const txHash = await refundBet(marketId);
    ...
    const isBusy = isPending || isEncrypting || isConfirming;
